...
class DQNAgent:
    ...
    def rebuild_model(self, n_nodes, n_actions):
        self.n_nodes = n_nodes
        self.n_actions = n_actions
        self.model = self._build_model()
...
def retrain_dqn_agent(env, agent, n_episodes=100):
    # Rebuild the model to match the new environment's topology
    agent.rebuild_model(n_nodes=env.n_nodes, n_actions=env.n_nodes)
    for episode in range(n_episodes):
        state = env.reset()
        state = preprocess_state(state, env.n_nodes)
        total_reward = 0
...
# Retrain the agent with the new environment
retrain_dqn_agent(new_env, agent)
