...
class NetworkEnvironment:
    def __init__(self, topology, destination_node):
...
class DQNAgent:
    ...
    def _build_model(self):
        model = Sequential()
        model.add(Dense(64, input_shape=(self.n_nodes,), activation='relu'))
    ...
    def act(self, state, valid_actions):
        ...
        invalid_actions = list(set(range(self.n_actions)) - set(valid_actions))
        act_values[0][invalid_actions] = -np.inf
...
env = NetworkEnvironment(topology, destination_node=4)
n_nodes = env.n_nodes
...
# Test the agent with the same environment
start_node = 0
destination_node = 4
